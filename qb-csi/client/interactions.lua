...
local function IsTargetDead(playerId)
    local retval = false
    QBCore.Functions.TriggerCallback('csi:server:isPlayerDead', function(result)
        retval = result
    end, playerId)
    Wait(100)
    return retval
...
RegisterNetEvent('csi:client:SetOutVehicle', function()
    local ped = PlayerPedId()
...
RegisterNetEvent('csi:client:PutInVehicle', function()
    local ped = PlayerPedId()
...
RegisterNetEvent('csi:client:SearchPlayer', function()
    local player, distance = QBCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        TriggerServerEvent("inventory:server:OpenInventory", "otherplayer", playerId)
        TriggerServerEvent("csi:server:SearchPlayer", playerId)
    else
        QBCore.Functions.Notify("No one nearby!", "error")
    end
end)

RegisterNetEvent('csi:client:SeizeCash', function()
    local player, distance = QBCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        TriggerServerEvent("csi:server:SeizeCash", playerId)
    else
        QBCore.Functions.Notify("No one nearby!", "error")
    end
end)

RegisterNetEvent('csi:client:SeizeDriverLicense', function()
    local player, distance = QBCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        TriggerServerEvent("csi:server:SeizeDriverLicense", playerId)
    else
        QBCore.Functions.Notify("No one nearby!", "error")
    end
end)
...
RegisterNetEvent('csi:client:RobPlayer', function()
    local player, distance = QBCore.Functions.GetClosestPlayer()
    local ped = PlayerPedId()
    if player ~= -1 and distance < 2.5 then
...
RegisterNetEvent('csi:client:JailCommand', function(playerId, time)
    TriggerServerEvent("csi:server:JailPlayer", playerId, tonumber(time))
end)

RegisterNetEvent('csi:client:BillCommand', function(playerId, price)
    TriggerServerEvent("csi:server:BillPlayer", playerId, tonumber(price))
end)

RegisterNetEvent('csi:client:JailPlayer', function()
    local player, distance = QBCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        ...
        if tonumber(dialog['jailtime']) > 0 then
            TriggerServerEvent("csi:server:JailPlayer", playerId, tonumber(dialog['jailtime']))
        else
...
RegisterNetEvent('csi:client:BillPlayer', function()
    local player, distance = QBCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        local dialog = exports['qb-input']:ShowInput({
        ...
        if tonumber(dialog['bill']) > 0 then
            TriggerServerEvent("csi:server:BillPlayer", playerId, tonumber(dialog['bill']))
        else
...
RegisterNetEvent('csi:client:PutPlayerInVehicle', function()
    local player, distance = QBCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        if not isHandcuffed and not isEscorted then
            TriggerServerEvent("csi:server:PutPlayerInVehicle", playerId)
        end
    else
        QBCore.Functions.Notify("No one nearby!", "error")
    end
...
RegisterNetEvent('csi:client:SetPlayerOutVehicle', function()
    local player, distance = QBCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        if not isHandcuffed and not isEscorted then
            TriggerServerEvent("csi:server:SetPlayerOutVehicle", playerId)
        end
    else
        QBCore.Functions.Notify("No one nearby!", "error")
    end
...
RegisterNetEvent('csi:client:EscortPlayer', function()
    local player, distance = QBCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        if not isHandcuffed and not isEscorted then
            TriggerServerEvent("csi:server:EscortPlayer", playerId)
        end
    else
        QBCore.Functions.Notify("No one nearby!", "error")
    end
...
RegisterNetEvent('csi:client:KidnapPlayer', function()
    local player, distance = QBCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        if not IsPedInAnyVehicle(GetPlayerPed(player)) then
            if not isHandcuffed and not isEscorted then
                TriggerServerEvent("csi:server:KidnapPlayer", playerId)
            end
        end
    else
        QBCore.Functions.Notify("No one nearby!", "error")
...
RegisterNetEvent('csi:client:CuffPlayerSoft', function()
    if not IsPedRagdoll(PlayerPedId()) then
        local player, distance = QBCore.Functions.GetClosestPlayer()
        if player ~= -1 and distance < 1.5 then
            local playerId = GetPlayerServerId(player)
            if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(PlayerPedId()) then
                TriggerServerEvent("csi:server:CuffPlayer", playerId, true)
                HandCuffAnimation()
            else
...
RegisterNetEvent('csi:client:CuffPlayer', function()
    if not IsPedRagdoll(PlayerPedId()) then
        local player, distance = QBCore.Functions.GetClosestPlayer()
        if player ~= -1 and distance < 1.5 then
            QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
                if result then 
                    local playerId = GetPlayerServerId(player)
                    if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(PlayerPedId()) then
                        TriggerServerEvent("csi:server:CuffPlayer", playerId, false)
                        HandCuffAnimation()
                    else
...
RegisterNetEvent('csi:client:GetEscorted', function(playerId)
    local ped = PlayerPedId()
...
RegisterNetEvent('csi:client:DeEscort', function()
    isEscorted = false
    TriggerEvent('hospital:client:isEscorted', isEscorted)
    DetachEntity(PlayerPedId(), true, false)
end)

RegisterNetEvent('csi:client:GetKidnappedTarget', function(playerId)
    local ped = PlayerPedId()
    QBCore.Functions.GetPlayerData(function(PlayerData)
...
RegisterNetEvent('csi:client:GetKidnappedDragger', function(playerId)
    QBCore.Functions.GetPlayerData(function(PlayerData)
...
RegisterNetEvent('csi:client:GetCuffed', function(playerId, isSoftcuff)
    local ped = PlayerPedId()
    if not isHandcuffed then
        isHandcuffed = true
        TriggerServerEvent("csi:server:SetHandcuffStatus", true)
        ClearPedTasksImmediately(ped)
        ...
        TriggerEvent('hospital:client:isEscorted', isEscorted)
        DetachEntity(ped, true, false)
        TriggerServerEvent("csi:server:SetHandcuffStatus", false)
        ClearPedTasksImmediately(ped)
        TriggerServerEvent("InteractSound_SV:PlayOnSource", "Uncuff", 0.2)
