    ...
    if coords then
        QBCore.Functions.SpawnVehicle(vehicle.vehicle, function(veh)
            QBCore.Functions.TriggerCallback('qb-garage:server:GetVehicleProperties', function(properties)
                ...
                TriggerServerEvent('csi:server:TakeOutImpound',vehicle.plate)
                closeMenuFull()
...
local function IsArmoryWhitelist() -- being removed
    ...
    if QBCore.Functions.GetPlayerData().job.name == 'csi' then
        retval = true
    end
    return retval
end
...
function MenuGarage(currentSelection)
    ...
    for veh, label in pairs(authorizedVehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            ...
            txt = "",
            params = {
                event = "csi:client:TakeOutVehicle",
                args = {
                    vehicle = veh,
...
function MenuImpound(currentSelection)
    ...
    QBCore.Functions.TriggerCallback("csi:GetImpoundedVehicles", function(result)
        ...
        if result == nil then
            ...
            for _ , v in pairs(result) do
                ...
                impoundMenu[#impoundMenu+1] = {
                    ...
                    params = {
                        event = "csi:client:TakeOutImpound",
                        args = {
                            vehicle = v,
                            currentSelection = currentSelection
                        }
...
RegisterNetEvent('csi:client:showFingerprint', function(playerId)
    openFingerprintUI()
    FingerPrintSessionId = playerId
end)

RegisterNetEvent('csi:client:showFingerprintId', function(fid)
    SendNUIMessage({
...
RegisterNUICallback('doFingerScan', function(data)
    TriggerServerEvent('csi:server:showFingerprintId', FingerPrintSessionId)
end)

RegisterNetEvent('csi:client:SendEmergencyMessage', function()
    TriggerServerEvent('csi:server:csiAlert', '| 10-99 | Officer in Distress')
    TriggerEvent("csi:client:CallAnim")
end)

RegisterNetEvent('csi:client:EmergencySound', function()
    PlaySound(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0, 0, 1)
end)

RegisterNetEvent('csi:client:CallAnim', function()
...
RegisterNetEvent('csi:client:ImpoundVehicle', function(fullImpound, price)
    ...
    if vehicle ~= 0 and vehicle then
        ...
        if #(pos - vehpos) < 5.0 and not IsPedInAnyVehicle(ped) then
            ...
            TriggerServerEvent("csi:server:Impound", plate, fullImpound, price, bodyDamage, engineDamage, totalFuel)
            QBCore.Functions.DeleteVehicle(vehicle)
        end
    end
end)

RegisterNetEvent('csi:client:CheckStatus', function()
    QBCore.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.job.name == "csi" then
            local player, distance = GetClosestPlayer()
            if player ~= -1 and distance < 5.0 then
                local playerId = GetPlayerServerId(player)
                QBCore.Functions.TriggerCallback('csi:GetPlayerStatus', function(result)
                    if result then
...
RegisterNetEvent("csi:client:VehicleMenuHeader", function (data)
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
...
RegisterNetEvent("csi:client:ImpoundMenuHeader", function (data)
...
RegisterNetEvent('csi:client:TakeOutImpound', function(data)
    local pos = GetEntityCoords(PlayerPedId())
    local takeDist = Config.Locations['impound'][data.currentSelection]
    takeDist = vector3(takeDist.x, takeDist.y,  takeDist.z)
    if #(pos - takeDist) <= 1.5 then
...
RegisterNetEvent('csi:client:TakeOutVehicle', function(data)
    local pos = GetEntityCoords(PlayerPedId())
    local takeDist = Config.Locations['vehicle'][data.currentSelection]
    takeDist = vector3(takeDist.x, takeDist.y,  takeDist.z)
    if #(pos - takeDist) <= 1.5 then
...
RegisterNetEvent('csi:client:EvidenceStashDrawer', function(data)
...
CreateThread(function()
    while true do
        local sleep = 2000
        if LocalPlayer.state.isLoggedIn and PlayerJob.name == "csi" then
            local pos = GetEntityCoords(PlayerPedId())
                ...
                if #(pos - v) < 5 then
                    ...
                    if #(pos - v) < 1.5 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            onDuty = not onDuty
                            TriggerServerEvent("csi:server:UpdateCurrentCops")
                            TriggerServerEvent("QBCore:ToggleDuty")
                            TriggerServerEvent("csi:server:UpdateBlips")
                        end
                            ...
                            exports['qb-menu']:showHeader({
                                {
                                    ...
                                    params = {
                                        event = 'csi:client:EvidenceStashDrawer',
                                        args = {
                                            currentEvidence = k
                                        }
                                    }
                ...
                if #(pos - v) < 4.5 then
                    if onDuty then
                        ...
                        if #(pos - v) < 1.5 then
                            ...
                            if IsControlJustReleased(0, 38) then
                                TriggerServerEvent("inventory:server:OpenInventory", "stash", "csistash_"..QBCore.Functions.GetPlayerData().citizenid)
                                TriggerEvent("inventory:client:SetCurrentStash", "csistash_"..QBCore.Functions.GetPlayerData().citizenid)
                            end
                        elseif #(pos - v) < 2.5 then
            ...
            for k, v in pairs(Config.Locations["trash"]) do
                if #(pos - v) < 2 then
                    ...
                    if #(pos - v) < 1.0 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            TriggerServerEvent("inventory:server:OpenInventory", "stash", "csitrash", {
                                maxweight = 4000000,
                                slots = 300,
                            })
                            TriggerEvent("inventory:client:SetCurrentStash", "csitrash")
                        end
                    elseif #(pos - v) < 1.5 then
                        ...
                        if #(pos - v) < 1.5 then
                            ...
                            if IsControlJustReleased(0, 38) then
                                ...
                                if player ~= -1 and distance < 2.5 then
                                    local playerId = GetPlayerServerId(player)
                                    TriggerServerEvent("csi:server:showFingerprint", playerId)
                                else
                                    QBCore.Functions.Notify("No one nearby!", "error")
                                end
                            end
                    ...
                    if onDuty then
                        ...
                        if #(pos - v) < 1.5 then
                            ...
                            if IsControlJustReleased(0, 38) then
                                ...
                                SetWeaponSeries()
                                TriggerServerEvent("inventory:server:OpenInventory", "shop", "csi", authorizedItems)
                            end
                        elseif #(pos - v) < 2.5 then
                    ...
                    if onDuty then
                        ...
                        if #(pos - vector3(v.x, v.y, v.z)) <= 1.5 then
                            if IsPedInAnyVehicle(PlayerPedId(), false) then
                                    ...
                                    exports['qb-menu']:showHeader({
                                        {
                                            header = 'csi Impound',
                                            params = {
                                                event = 'csi:client:ImpoundMenuHeader',
                                                args = {
                                                    currentSelection = k,
                                                }
                                            }
                                ...
                                -- DrawText3D(v.x, v.y, v.z, "~g~E~w~ - csi Impound")
                            end
                            if IsControlJustReleased(0, 38) then
                                if IsPedInAnyVehicle(PlayerPedId(), false) then
                                    QBCore.Functions.DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
                                    ...
                                    exports['qb-menu']:showHeader({
                                        {
                                            header = 'csi Garage',
                                            params = {
                                                event = 'csi:client:VehicleMenuHeader',
                                                args = {
                                                    currentSelection = k,
                                                }
                                            }
